...
var (
	...
	joinLongDescription = dedent.Dedent(`
		When joining a edgeadm initialized cluster, we need to establish
...
func NewJoinCMD(out io.Writer, edgeConfig *cmd.EdgeadmConfig) *cobra.Command {
	...
	// binds the Runner to edgeadm join command by altering
...
func joinConfigFlags(flagSet *flag.FlagSet, edgeConfig *cmd.EdgeadmConfig) {
	flagSet.StringVar(
