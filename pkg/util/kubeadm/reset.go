...
func NewResetCMD(in io.Reader, out io.Writer, edgeConfig *cmd.EdgeadmConfig) *cobra.Command {
	...
	cmds := &cobra.Command{
		Use:   "reset",
		Short: "Performs a best effort revert of changes made to this host by 'edgeadm init' or 'edgeadm join'",
	...
	AddResetFlags(cmds.Flags(), resetOptions)
	...
	if edgeConfig.IsEnableEdge {
		resetRunner.AppendPhase(steps.NewCleanupLiteApiServerPhase())
	}
	...
	resetRunner.BindToCommand(cmds)
